...
import GROUPED_ANIMATION_TYPES from '@const/grouped-animation-types.json';
...
const handleRowPressed = (componentRef, animationType) => {
    componentRef.setNativeProps({
        style: {
            zIndex: 1,
        },
    });
    componentRef.animate(animationType, state.duration).then(() => {
        componentRef.setNativeProps({
            style: {
                zIndex: 0,
            },

...
const CameraScreen: FunctionComponent = (props: any) => {
  const [showListMenuComponentCamera, setStateShowListMenuComponent] = useState(false);
  const [listItemCameraRender, setStateListItemCameraRender] = useState([]);
  ...
  const listItemCameraRenderLast = useRef([]);

  useEffect(() => {
    const backAction = () => {
      if (renderAnimated && Array.isArray(listItemCameraRenderLast.current) && listItemCameraRenderLast.current.length > 0) {
        setStateRenderAnimated(false);
        setStateListItemCameraRender(listItemCameraRenderLast.current);
      }
      if (showListMenuComponentCamera) {
        if (renderAnimated) {
  ...
  }, [showListMenuComponentCamera, renderAnimated]);
  ...
  const renderContent = () => {
      ...
      return (
        <SafeAreaView style={styles.content}>
          <View style={[{ minHeight: SizeRpScreen.height(100), width: SizeRpScreen.width(100) }]}>
            {Array.isArray(listItemCameraRender) && listItemCameraRender.map((itemRenderAnimated: any, indexRenderAnimated: number) => {
              return (
                <ViewAnimated
    ...
    return (
      <SafeAreaView style={styles.content}>
        {Array.isArray(MenuCameras) && MenuCameras.map((itemMenuAnimated: any, index: number) => {
          return (
            <ViewAnimated
              key={`${index}`}
              animation="fadeIn"
            ...
            >
              <TouchableOpacity
                onPress={() => {
                  setStateShowListMenuComponent(true);
                  setStateListItemCameraRender(itemMenuAnimated.data);
                  listItemCameraRenderLast.current = itemMenuAnimated.data;
                }}
                style={styles.buttonActionsMenu}>
                <AppText style={{}}>
